...
As always, the trick here is to choose the right scale factors.  The identity that we're looking for comes from equality of the top and bottom sides of the rectangle.
...
Since $K = rs$ if we multiply both sides by $s$ we get
\[ sxyz = rs \cdot r \cdot s = \mathcal{A}^2  \]
Plugging in for $x = s - a$ and so on gives Heron's formula.
