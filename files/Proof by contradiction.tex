...
Theorem:  if there is an isosceles triangle with rational sides, then there is a smaller one with the same property.
...
Furthermore, the \emph{other} small triangle is also isosceles.  Its base angles, when added to the equal angles of an isosceles triangle, form right angles.  This allows us to mark the side along the base as having length $x$ as well.  (One can also draw the circle with center at the top vertex and of unit radius, then these two lengths are tangents to that circle from the same point).

Thus, the hypotenuse of the new, small right triangle is a rational number, since it is equal to $1 - x$.
...
This section isn't geometrical, but it still provides a really neat proof for the irrationality of certain numbers.  When we investigated the pentagon, we ran into the number $\phi$ which satisfies this expression:
\[ \phi^2 = \phi + 1 \]
...
The dots indicate that this continues.  In fact, it continues forever, by the nature of the construction.  To evaluate $\phi$, we must chop off the dots at some point (the result is an approximation):
...
Returning to $\sqrt{2}$, we can do something similar:
\[ (\sqrt{2} + 1)(\sqrt{2} - 1) = 1 \]

I find it less distracting to substitute $x = \sqrt{2}$.
\[ (x + 1)(x - 1) = 1 \]

Rearrange in two different ways.
\[ x + 1 = \frac{1}{x - 1} \]
\[ x - 1 = \frac{1}{x + 1} \]
...
Substitute the whole right-hand side into itself as
\[ x = 1 + \cfrac{1}{2 + \cfrac{1}{x + 1}} \]

This can be repeated forever.  At any step we can obtain an approximation for $\sqrt{2}$, but we can never write it exactly as a fraction.

Here is a Python script to do the calculation:
